...
function Scan-Patches {
    ...
    if ($answer -notmatch '^(y|yes)$') {
        Log "INFO" "Patch declined; ordered intake stops for this session. No later patch will be considered."
        return
    }
    try {
        Apply-Patch $patch
...
function Menu {
    while ($true) {
        ...
        Write-Host ""
        Write-Host " 3. Run & play / ForgeGUI_Core Lab"
        ...
        switch ($choice) {
            ...
            "4" { Patch-Status }
            "5" { Project-Health }
            "6" { [void](Package-Debug "manual handoff requested" "MANUAL" $true) }
            "7" { Advanced-Menu }
            "0" { return }
            default { Write-Host "Unknown option" }
        }
...
if ([string]::IsNullOrWhiteSpace($Operation)) {
    ...
    if ($script:RestartRequired) {
        Log "PASS" "Patch session complete. PCC provider/source authority must be reloaded; exiting cleanly for restart."
        exit 0
    }
    Root-Hygiene
    ...
    [void](Stage-RootPatchArchive)
    Scan-Patches
    if ($script:RestartRequired) { exit 0 }
} else {
