...
pub fn tool_button(
    ...
    .corner_radius(control_radius(theme));
    ui.add(button)
}
...
/// Small, consistently sized actions shared by docked and floating panel headers.
pub fn panel_chrome_button(
    ui: &mut Ui,
    glyph: &str,
    tooltip: &str,
    active: bool,
    ...
    let fill = if active {
        ...
        theme.chrome.panel_header
    };
    ui.add(
        Button::new(RichText::new(glyph).size(13.0).color(color(foreground)))
            .selected(active)
            .fill(color(fill))
            .stroke(Stroke::new(
...
pub fn badge(ui: &mut Ui, text: &str, tone: WidgetTone, theme: &ForgeTheme) -> Response {
    ...
    ui.add(
        Button::new(RichText::new(text).small().color(color(foreground)))
            ...
            .corner_radius(control_radius(theme).max(8))
            .frame(true),
    )
}
...
fn control_radius(theme: &ForgeTheme) -> u8 {
    theme.interaction.control_radius.clamp(0.0, u8::MAX as f32) as u8
}
