...
Duplicate catalog registration must be rejected before mutation. Patch intake remains ordered and transactional, but provider 0.4.13 can drain an approved queue in-process. Ordinary source patches do not require a restart; PCC-source updates automatically relaunch the refreshed provider in the same console.
...
## Universal modular-shell finding

ForgeGUI must not identify itself with an editor-shaped canvas. The universal shell defaults to generic application content and composes independent left/center/right/bottom/floating surfaces, tab stacks, movable toolbars, menus, status chrome, and optional custom-render surfaces. Canvas/viewport tooling remains an opt-in authoring profile.

The application metadata and canonical Lab now use the universal application profile. GameMaker/editor-inspired presentation remains a consumer styling option rather than the framework default.

## CI finding

The workspace retired `forge_gui_next_lab` and `forge_authoring_lab`, but the CI workflow still attempted to build both removed packages. The clean reconstruction replaces those stale jobs with the current certification set: Contracts Smoke, Micro Lab, App Lab, and canonical ForgeGUI Lab.
